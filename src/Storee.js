...
import StoreimgesModal from './StoreimgesModal';
...
const imageClose =() => setModalimages(false)
    ...
    if(productdata.length < 1){
        ...
        .then((resolve)=>{
            dispatch(addData((resolve.data.reduce((out, inn)=>{
                out.push({id:inn.id ,title : inn.title , price : inn.price , description : inn.description , image : inn.images , category : inn.category.name,  qty : 1 ,customprice : inn.price})
...
<div className='productbg'>
    <div className='hearts'>
       {!whis?.includes(ele?.id) && <img className='heart1' src={heart1} onClick={()=>{dispatch(addarr([...whis , ele?.id]))}} ></img>}
        ...
        <div><img src={eye} onClick={()=>{setmodal(true); setmodaldata(ele?.id)}} className='eyeimg'></img></div>
    </div>

                    <img className='productimg' onClick={()=>{setModalimages(true); setimageId(ele?.id)}}  src={ele?.image[0]}></img> 
                        <p>{ele?.title}</p>
                        <p>&#8377; {ele?.price*79.80}</p>
           {!duplicat?.includes(ele?.id) &&  <button className='btn bg-warning addtocart' onClick={()=>{!duplicat.includes(ele?.id) && dispatch(reducedata([...duplicat,ele?.id]));}}>Add to Cart</button>}
            ...
            <StoreimgesModal close={imageClose} id={imageId}  show={Modalimages}/>
        </div>
    )
